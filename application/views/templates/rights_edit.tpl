			...
			<div class="span12">
				<div class="widget stacked">
					...
					<div class="widget-content">
						<form class="form-horizontal" action="<{$baseurl}>rights/edit?rightsid=<{$rightsid}>" method="post" name="theForm" enctype="multipart/form-data">
							<fieldset>
							  	<div class="control-group">
									<label class="control-label" for="focusedInput">权限名称</label>
									<div class="controls">
								  		<input class="input-xlarge focused" type="text" name="rightsname" value="<{$rightsinfo.rightsname}>">
									</div>
							  	</div>
							  	<div class="control-group">
									<label class="control-label" for="focusedInput">Action</label>
									<div class="controls">
								  		<textarea class="input-xlarge" rows="6" name="action"><{$rightsinfo.action}></textarea>
									</div>
							  	</div>
								<div class="control-group">
									...
									<div class="controls">
										<select name="parentid" class="input-xlarge" id="parentid" onchange=ShowChildMenu()>
											...
											<{foreach from=$parentmenu item=i}>
												<option value="<{$i.menuid}>" <{if $i.menuid == $rightsinfo.parentid}>selected<{/if}>><{$i.menuname}></option>
											<{/foreach}>
										</select>
									</div>
								</div>
								<div class="control-group" <{if !$childmenu}>style="display:none;"<{/if}> id="childmenu">
									...
									<div class="controls">
										<select name="menuid" class="input-xlarge" id="menuid">
											...
											<{foreach from=$childmenu item=i}>
												<option value="<{$i.menuid}>" <{if $i.menuid == $rightsinfo.menuid}>selected<{/if}>><{$i.menuname}></option>
											<{/foreach}>
										</select>
									</div>
								</div>
